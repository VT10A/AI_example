...
if filter_type == "Filtered":
        ...
        heading = get_chat_completions(f"Summarise the top topics in two sentences, in the format X, Y, and Z are the top 5 (five) cited themes, accounting for x% of the total mentions(no decimals). Here's the data {topic_percentages}. Don't comment on the 'Other' mentions.")
        st.write(heading)
...
#    age_summary = get_age_summary(f"Summarise any intereseting differences by age within two paragraphs, focusing on the percentages of the total mentions (no decimals). Here's the data {age_group_topic_percentages}. Don't comment on the 'Other' mentions.")
    age_summary = get_age_summary(f"Summarise any intereseting differences by age between those aged 18-34yrs verses 55yrs+ within two paragraphs, focusing on the percentages of the total mentions (no decimals). For example Atmosphere seems more important to the 55+. Here's the data {age_group_topic_percentages}. Don't comment on the 'Other' mentions.")
